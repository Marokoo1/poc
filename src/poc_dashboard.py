from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
from typing import Iterable

import numpy as np
import pandas as pd
import plotly.graph_objects as go
import streamlit as st
import yaml

from ib_calculator import calculate_all_ib_levels

STANDARD_OHLCV = ["Date", "Open", "High", "Low", "Close", "Volume"]
DISPLAY_MODE_OPTIONS = ["POC", "IB", "POC + IB"]
POC_PERIODS = ["weekly", "monthly", "yearly"]
IB_PERIODS = ["monthly_ib", "yearly_ib", "monthly_ib_fib", "yearly_ib_fib"]

STYLE_MAP = {
    "weekly": {"label": "Weekly POC", "color": "#6EC1FF", "width": 1.4, "dash": "dot", "rank": 6},
    "monthly": {"label": "Monthly POC", "color": "#FFB84D", "width": 2.2, "dash": "dash", "rank": 5},
    "yearly": {"label": "Yearly POC", "color": "#FF6B6B", "width": 3.0, "dash": "solid", "rank": 4},
    "monthly_ib": {"label": "Monthly IB", "color": "#A78BFA", "width": 2.0, "dash": "dash", "rank": 3},
    "yearly_ib": {"label": "Yearly IB", "color": "#10B981", "width": 2.4, "dash": "solid", "rank": 2},
    "monthly_ib_fib": {"label": "Monthly IB Fib", "color": "#F472B6", "width": 1.6, "dash": "dot", "rank": 8},
    "yearly_ib_fib": {"label": "Yearly IB Fib", "color": "#EC4899", "width": 1.8, "dash": "dot", "rank": 7},
}

@dataclass
class DataSource:
    raw_data_dir: Path
    processed_data_dir: Path
    raw_file_pattern: str = "{ticker}.csv"
    poc_file_pattern: str = "{ticker}_poc.csv"

@dataclass
class ChartSettings:
    display_mode: str
    selected_poc_periods: list[str]
    selected_ib_periods: list[str]
    selected_years: list[int]
    selected_months: list[int]
    level_status: str
    show_labels: bool
    nearest_only: bool
    nearest_count: int
    months_back: int
    extend_from_activation: bool
    show_confluence_only: bool
    confluence_max_atr: float


def normalize_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    rename_map = {}
    for col in df.columns:
        key = str(col).strip().lower()
        if key in {"datetime", "date"}:
            rename_map[col] = "Date"
        elif key in {"open", "high", "low", "close", "volume"}:
            rename_map[col] = key.capitalize()
    df = df.rename(columns=rename_map)
    missing = [col for col in STANDARD_OHLCV if col not in df.columns]
    if missing:
        raise ValueError(f"OHLCV soubor nemá očekávané sloupce: {missing}")
    out = df[STANDARD_OHLCV].copy()
    out["Date"] = pd.to_datetime(out["Date"], errors="coerce")
    for col in ["Open", "High", "Low", "Close", "Volume"]:
        out[col] = pd.to_numeric(out[col], errors="coerce")
    out = out.dropna(subset=STANDARD_OHLCV).sort_values("Date").reset_index(drop=True)
    return out


@st.cache_data(show_spinner=False, ttl=60)
def load_yaml_settings(settings_path: str) -> dict:
    path = Path(settings_path)
    if not path.exists():
        return {}
    with path.open("r", encoding="utf-8") as f:
        return yaml.safe_load(f) or {}


@st.cache_data(show_spinner=False, ttl=60)
def load_ohlcv_from_csv(raw_file_path: str) -> pd.DataFrame:
    path = Path(raw_file_path)
    if not path.exists():
        raise FileNotFoundError(f"Raw data soubor nebyl nalezen: {path}")
    df = pd.read_csv(path)
    return normalize_ohlcv(df)


@st.cache_data(show_spinner=False, ttl=60)
def load_poc_data(poc_file_path: str, ticker_hint: str | None = None) -> pd.DataFrame:
    path = Path(poc_file_path)
    if not path.exists():
        raise FileNotFoundError(f"POC soubor nebyl nalezen: {path}")
    df = pd.read_csv(path)
    for col in ["PeriodStart", "PeriodEnd"]:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], errors="coerce")
    if "Ticker" not in df.columns:
        df["Ticker"] = str(ticker_hint or path.stem.replace("_poc", "")).upper().strip()
    required = {"Ticker", "PeriodType", "Period", "PeriodStart", "PeriodEnd", "POC"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"POC CSV nemá očekávané sloupce: {sorted(missing)}")
    df["Ticker"] = df["Ticker"].astype(str).str.upper().str.strip()
    df["PeriodType"] = df["PeriodType"].astype(str).str.lower().str.strip()
    df["POC"] = pd.to_numeric(df["POC"], errors="coerce")
    return df.dropna(subset=["Ticker", "PeriodType", "POC", "PeriodStart", "PeriodEnd"]).copy()


@st.cache_data(show_spinner=False, ttl=60)
def compute_ib_levels_from_ohlcv(raw_file_path: str, ticker: str, ib_settings_dict: dict) -> pd.DataFrame:
    ohlcv = load_ohlcv_from_csv(raw_file_path)
    return calculate_all_ib_levels(ohlcv.copy(), ticker=ticker, settings=ib_settings_dict)


def discover_project_root() -> Path:
    here = Path(__file__).resolve().parent
    return here.parent


def build_source(project_root: Path, settings: dict) -> DataSource:
    paths_cfg = settings.get("paths", {})
    raw_dir = paths_cfg.get("raw_data_dir", "data/raw")
    processed_dir = paths_cfg.get("processed_data_dir", "data/processed")
    csv_cfg = settings.get("csv", {})
    return DataSource(
        raw_data_dir=(project_root / raw_dir).resolve(),
        processed_data_dir=(project_root / processed_dir).resolve(),
        raw_file_pattern=str(csv_cfg.get("file_pattern", "{symbol}.csv")).replace("{symbol}", "{ticker}"),
        poc_file_pattern="{ticker}_poc.csv",
    )


def list_tickers(processed_dir: Path, poc_file_pattern: str) -> list[str]:
    base = processed_dir
    if not base.exists():
        return []
    suffix = poc_file_pattern.replace("{ticker}", "")
    prefix = poc_file_pattern.split("{ticker}")[0] if "{ticker}" in poc_file_pattern else ""
    tickers = []
    for file_path in sorted(base.glob("*.csv")):
        name = file_path.name
        if prefix and not name.startswith(prefix):
            continue
        if suffix and not name.endswith(suffix):
            continue
        ticker = name[len(prefix): len(name) - len(suffix) if suffix else None]
        if ticker:
            tickers.append(ticker.upper())
    return sorted(set(tickers))


def infer_ib_side(badge: str, level_name: str, multiplier) -> str:
    try:
        if multiplier is not None and not pd.isna(multiplier):
            return "support" if float(multiplier) <= 0 else "resistance"
    except Exception:
        pass
    b = str(badge or "").strip()
    if b == "0" or b.startswith("-"):
        return "support"
    name = str(level_name).upper()
    if "NEG" in name:
        return "support"
    return "resistance"


def enrich_set_time_columns(levels: pd.DataFrame) -> pd.DataFrame:
    out = levels.copy()
    out["SetYear"] = pd.Series(dtype="Int64")
    out["SetMonth"] = pd.Series(dtype="Int64")
    out["SetLabel"] = pd.Series(dtype="string")
    if out.empty:
        return out
    years = []
    months = []
    labels = []
    for _, row in out.iterrows():
        period_type = str(row.get("PeriodType", "")).lower()
        period = str(row.get("Period", "")).strip()
        y = pd.NA
        m = pd.NA
        if "yearly" in period_type:
            try:
                y = int(period[:4])
            except Exception:
                pass
            lbl = str(period)
        elif "monthly" in period_type:
            try:
                y_s, m_s = period.split("-")
                y = int(y_s)
                m = int(m_s)
                lbl = f"{int(y):04d}-{int(m):02d}"
            except Exception:
                lbl = period
        else:
            lbl = period
        years.append(y)
        months.append(m)
        labels.append(lbl)
    out["SetYear"] = pd.Series(years, dtype="Int64")
    out["SetMonth"] = pd.Series(months, dtype="Int64")
    out["SetLabel"] = pd.Series(labels, dtype="string")
    return out


def prepare_poc_levels(levels: pd.DataFrame, ohlcv: pd.DataFrame) -> pd.DataFrame:
    if levels.empty:
        return pd.DataFrame()
    out = levels.copy()
    prices = ohlcv.copy().set_index("Date")
    last_close = float(prices["Close"].iloc[-1])
    touch_dates = []
    touched = []
    for _, row in out.iterrows():
        after = prices[prices.index > pd.Timestamp(row["PeriodEnd"])]
        mask = (after["Low"] <= row["POC"]) & (after["High"] >= row["POC"])
        if mask.any():
            d = pd.Timestamp(after.index[mask][0])
            touch_dates.append(d)
            touched.append(True)
        else:
            touch_dates.append(pd.NaT)
            touched.append(False)
    out["Source"] = "POC"
    out["LevelPrice"] = pd.to_numeric(out["POC"], errors="coerce")
    out["AnchorStart"] = pd.to_datetime(out["PeriodEnd"], errors="coerce")
    out["TouchDate"] = touch_dates
    out["Touched"] = touched
    out["Valid"] = ~pd.Series(touched)
    out["Status"] = np.where(out["Valid"], "active", "tested")
    out["DisplayUntil"] = out["TouchDate"]
    out["LastPrice"] = last_close
    out["SignedDist"] = (last_close - out["LevelPrice"]).round(4)
    out["AbsDist"] = out["SignedDist"].abs().round(4)
    out["PeriodRank"] = out["PeriodType"].map(lambda x: STYLE_MAP.get(x, {}).get("rank", 99))
    return enrich_set_time_columns(out)


def prepare_ib_levels(levels: pd.DataFrame, ohlcv: pd.DataFrame) -> pd.DataFrame:
    if levels.empty:
        return pd.DataFrame()
    out = levels.copy()
    prices = ohlcv.copy().sort_values("Date").reset_index(drop=True)
    last_close = float(prices["Close"].iloc[-1])
    out["Source"] = "IB"
    out["AnchorStart"] = pd.to_datetime(out["ActiveFrom"], errors="coerce")
    out["LevelPrice"] = pd.to_numeric(out["LevelPrice"], errors="coerce")

    touch_dates = []
    touches = []
    valids = []
    for _, row in out.iterrows():
        active_from = row.get("AnchorStart")
        price = row.get("LevelPrice")
        side = infer_ib_side(row.get("LevelBadge"), row.get("LevelName"), row.get("Multiplier"))
        if pd.isna(active_from) or pd.isna(price):
            touch_dates.append(pd.NaT); touches.append(0); valids.append(True); continue
        future = prices[prices["Date"] >= active_from]
        if future.empty:
            touch_dates.append(pd.NaT); touches.append(0); valids.append(True); continue
        if side == "resistance":
            hits = future[future["High"] >= price]
        else:
            hits = future[future["Low"] <= price]
        if hits.empty:
            touch_dates.append(pd.NaT); touches.append(0); valids.append(True)
        else:
            touch_dates.append(pd.Timestamp(hits.iloc[0]["Date"])); touches.append(int(len(hits))); valids.append(False)
    out["TouchDate"] = touch_dates
    out["TouchesCount"] = touches
    out["Valid"] = valids
    out["Status"] = np.where(out["Valid"], "active", "tested")
    out["DisplayUntil"] = out["TouchDate"]
    out["LastPrice"] = last_close
    out["SignedDist"] = (last_close - out["LevelPrice"]).round(4)
    out["AbsDist"] = out["SignedDist"].abs().round(4)
    out["PeriodRank"] = out["PeriodType"].map(lambda x: STYLE_MAP.get(x, {}).get("rank", 99))
    return enrich_set_time_columns(out)


def apply_confluence_filter(levels: pd.DataFrame, max_atr: float) -> pd.DataFrame:
    if levels.empty:
        return levels.copy()
    poc = levels[levels["Source"] == "POC"].copy()
    ib = levels[levels["Source"] == "IB"].copy()
    if poc.empty or ib.empty:
        return pd.DataFrame(columns=levels.columns)
    all_prices = pd.concat([poc["LevelPrice"], ib["LevelPrice"]], ignore_index=True)
    price_scale = float(all_prices.std()) if len(all_prices) > 1 else 0.0
    if not pd.notna(price_scale) or price_scale <= 0:
        price_scale = max(float(all_prices.abs().median()) * 0.01, 0.01)
    max_abs_distance = max_atr * price_scale
    keep_poc, keep_ib = set(), set()
    for poc_idx, poc_row in poc.iterrows():
        distances = (ib["LevelPrice"] - float(poc_row["LevelPrice"])).abs()
        matched = ib[distances <= max_abs_distance]
        if not matched.empty:
            keep_poc.add(poc_idx)
            keep_ib.update(matched.index.tolist())
    return pd.concat([poc.loc[sorted(keep_poc)], ib.loc[sorted(keep_ib)]], ignore_index=True)


def filter_unified_levels(levels: pd.DataFrame, settings: ChartSettings) -> pd.DataFrame:
    if levels.empty:
        return levels.copy()
    data = levels.copy()
    if settings.display_mode == "POC":
        allowed_sources = ["POC"]
    elif settings.display_mode == "IB":
        allowed_sources = ["IB"]
    else:
        allowed_sources = ["POC", "IB"]
    data = data[data["Source"].isin(allowed_sources)].copy()
    if "POC" in allowed_sources:
        poc_mask = data["Source"].eq("POC")
        data = pd.concat([data[poc_mask & data["PeriodType"].isin(settings.selected_poc_periods)], data[~poc_mask]], ignore_index=True)
    if "IB" in allowed_sources:
        ib_mask = data["Source"].eq("IB")
        ib_data = data[ib_mask].copy()
        if settings.selected_ib_periods:
            ib_data = ib_data[ib_data["PeriodType"].isin(settings.selected_ib_periods)].copy()
        def _match_period(row):
            period_type = str(row.get("PeriodType", "")).lower().strip()
            period = str(row.get("Period", "")).strip()
            if "yearly" in period_type:
                try:
                    y = int(period[:4])
                except Exception:
                    return False
                return y in settings.selected_years if settings.selected_years else True
            if "monthly" in period_type:
                try:
                    y_s, m_s = period.split("-")
                    y = int(y_s)
                    m = int(m_s)
                except Exception:
                    return False
                year_ok = y in settings.selected_years if settings.selected_years else True
                month_ok = m in settings.selected_months if settings.selected_months else True
                return year_ok and month_ok
            return True
        ib_data = ib_data[ib_data.apply(_match_period, axis=1)].copy()
        data = pd.concat([data[~ib_mask], ib_data], ignore_index=True)
    if settings.level_status == "Only active":
        data = data[data["Valid"]].copy()
    elif settings.level_status == "Only tested":
        data = data[~data["Valid"]].copy()
    if settings.display_mode == "POC + IB" and settings.show_confluence_only:
        data = apply_confluence_filter(data, settings.confluence_max_atr)
    if settings.nearest_only and not data.empty:
        data = data.nsmallest(settings.nearest_count, ["AbsDist", "PeriodRank"]).copy()
    return data.sort_values(["Source", "PeriodRank", "AbsDist", "AnchorStart"], ascending=[True, True, True, False]).reset_index(drop=True)


def format_level_badge(row: pd.Series) -> str:
    source = str(row.get("Source", "")).upper()
    if source != "IB":
        period_type = str(row.get("PeriodType", "")).lower().strip()
        period = str(row.get("Period", "")).strip()
        if period_type == "weekly":
            return f"W {period}"
        if period_type == "monthly":
            return f"M {period}"
        if period_type == "yearly":
            return f"Y {period}"
        return period
    period_type = str(row.get("PeriodType", "")).lower().strip()
    period = str(row.get("Period", "")).strip()
    badge = str(row.get("LevelBadge", "")).strip()
    prefix = "IB"
    if "yearly" in period_type:
        try:
            yy = int(period[:4]) % 100
            prefix = f"Y {yy:02d}"
        except Exception:
            prefix = f"Y {period}"
    elif "monthly" in period_type:
        try:
            yyyy, mm = period.split("-")
            yy = int(yyyy) % 100
            prefix = f"M{int(mm):02d} {yy:02d}"
        except Exception:
            prefix = f"M {period}"
    return f"{prefix} {badge}".strip()


def build_chart(ohlcv: pd.DataFrame, levels: pd.DataFrame, ticker: str, settings: ChartSettings) -> go.Figure:
    ohlcv_idx = ohlcv.copy().set_index("Date")
    history_start = ohlcv_idx.index.max() - pd.DateOffset(months=settings.months_back)
    chart_df = ohlcv_idx[ohlcv_idx.index >= history_start].copy()
    if chart_df.empty:
        chart_df = ohlcv_idx.copy()
    fig = go.Figure()
    fig.add_trace(go.Candlestick(
        x=chart_df.index, open=chart_df["Open"], high=chart_df["High"], low=chart_df["Low"], close=chart_df["Close"],
        name=ticker, increasing_line_color="#22C55E", decreasing_line_color="#EF4444", whiskerwidth=0.35, showlegend=False,
    ))
    legend_done = set()
    max_x = chart_df.index.max()
    for _, row in levels.iterrows():
        period_type = str(row.get("PeriodType", ""))
        style = STYLE_MAP.get(period_type, {"label": period_type, "color": "#94A3B8", "width": 1.5, "dash": "dot"})
        x0 = pd.Timestamp(row.get("AnchorStart")) if pd.notna(row.get("AnchorStart")) else chart_df.index.min()
        x1 = max_x
        if settings.extend_from_activation and pd.notna(row.get("DisplayUntil")):
            x1 = pd.Timestamp(row.get("DisplayUntil"))
        legend_name = style["label"]
        fig.add_shape(type="line", x0=x0, x1=x1, y0=row["LevelPrice"], y1=row["LevelPrice"], line=dict(color=style["color"], width=style["width"], dash=style["dash"]))
        if legend_name not in legend_done:
            fig.add_trace(go.Scatter(x=[None], y=[None], mode="lines", line=dict(color=style["color"], width=style["width"], dash=style["dash"]), name=legend_name))
            legend_done.add(legend_name)
        if settings.show_labels:
            fig.add_annotation(x=max_x, y=float(row["LevelPrice"]), xanchor="left", yanchor="middle", text=format_level_badge(row),
                               showarrow=False, font=dict(size=10, color=style["color"]), bgcolor="rgba(0,0,0,0.25)", bordercolor=style["color"], borderwidth=1)
    fig.update_layout(template="plotly_dark", height=760, title=f"{ticker} – Level dashboard ({settings.display_mode})", xaxis_rangeslider_visible=False, margin=dict(l=10, r=120, t=50, b=20), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0.0))
    fig.update_xaxes(showgrid=True, gridcolor="rgba(255,255,255,0.06)")
    fig.update_yaxes(showgrid=True, gridcolor="rgba(255,255,255,0.06)")
    return fig


def sidebar_controls(source: DataSource, tickers: Iterable[str]) -> tuple[str, ChartSettings, dict]:
    st.sidebar.header("Level Dashboard")
    display_mode = st.sidebar.radio("Režim", DISPLAY_MODE_OPTIONS, index=2)
    ticker = st.sidebar.selectbox("Ticker", list(tickers), index=0 if tickers else None)
    selected_poc_periods = st.sidebar.multiselect("POC periody", POC_PERIODS, default=["yearly", "monthly"])
    selected_ib_periods = st.sidebar.multiselect("IB typy", IB_PERIODS, default=["yearly_ib", "monthly_ib"])
    available_years = list(range(2015, 2031))
    selected_years = st.sidebar.multiselect("Rok sady", available_years, default=[2025])
    month_options = list(range(1, 13))
    selected_months = st.sidebar.multiselect("Měsíc sady", month_options, default=month_options)
    level_status = st.sidebar.selectbox("Stav levelu", ["All", "Only active", "Only tested"], index=0)
    nearest_only = st.sidebar.checkbox("Zobrazit jen nejbližší levely", value=False)
    nearest_count = st.sidebar.slider("Počet nejbližších levelů", min_value=2, max_value=40, value=10, step=1)
    months_back = st.sidebar.slider("Kolik měsíců historie v grafu", min_value=6, max_value=72, value=24, step=1)
    show_labels = st.sidebar.checkbox("Popisky levelů vpravo", value=True)
    extend_from_activation = st.sidebar.checkbox("Vést čáru od aktivace / konce periody", value=True)
    show_confluence_only = st.sidebar.checkbox("V režimu POC + IB ukázat jen konfluence", value=False)
    confluence_max_atr = st.sidebar.slider("Max vzdálenost pro konfluenci (proxy)", min_value=0.05, max_value=1.00, value=0.25, step=0.05)
    settings = ChartSettings(
        display_mode=display_mode, selected_poc_periods=selected_poc_periods, selected_ib_periods=selected_ib_periods,
        selected_years=selected_years, selected_months=selected_months, level_status=level_status, show_labels=show_labels,
        nearest_only=nearest_only, nearest_count=nearest_count, months_back=months_back,
        extend_from_activation=extend_from_activation, show_confluence_only=show_confluence_only, confluence_max_atr=float(confluence_max_atr)
    )
    ib_settings = {
        "enabled": True,
        "yearly_enabled": True,
        "monthly_enabled": True,
        "monthly_mode": "first_5_trading_days",
        "standard_projection_enabled": True,
        "standard_multipliers": [1.5, 2.0],
        "fibonacci_projection_enabled": False,
        "fibonacci_multipliers": [1.272, 1.618],
    }
    return ticker, settings, ib_settings


def make_paths(source: DataSource, ticker: str) -> tuple[Path, Path]:
    raw_path = source.raw_data_dir / source.raw_file_pattern.format(ticker=ticker)
    poc_path = source.processed_data_dir / source.poc_file_pattern.format(ticker=ticker)
    return raw_path, poc_path


def build_table_view(levels: pd.DataFrame) -> pd.DataFrame:
    if levels.empty:
        return pd.DataFrame()
    cols = [
        "Source", "PeriodType", "Period", "LevelName", "LevelBadge", "LevelPrice", "Status",
        "ActiveFrom", "TouchDate", "LastPrice", "SignedDist", "AbsDist"
    ]
    existing = [c for c in cols if c in levels.columns]
    out = levels[existing].copy()
    for c in ["ActiveFrom", "TouchDate"]:
        if c in out.columns:
            out[c] = pd.to_datetime(out[c], errors="coerce")
    return out


def main() -> None:
    st.set_page_config(page_title="Level Dashboard", layout="wide")
    project_root = discover_project_root()
    settings = load_yaml_settings(str(project_root / "config" / "settings.yaml"))
    source = build_source(project_root, settings)
    tickers = list_tickers(source.processed_data_dir, source.poc_file_pattern)
    if not tickers:
        st.error("Nebyly nalezeny žádné tickery v processed adresáři.")
        st.stop()
    ticker, chart_settings, ib_settings = sidebar_controls(source, tickers)
    raw_path, poc_path = make_paths(source, ticker)
    try:
        ohlcv = load_ohlcv_from_csv(str(raw_path))
    except Exception as e:
        st.error(f"Nepodařilo se načíst OHLCV: {e}")
        st.stop()
    poc_levels = pd.DataFrame()
    if poc_path.exists():
        try:
            poc_levels = load_poc_data(str(poc_path), ticker_hint=ticker)
        except Exception as e:
            st.warning(f"POC data se nepodařilo načíst: {e}")
    ib_levels = compute_ib_levels_from_ohlcv(str(raw_path), ticker=ticker, ib_settings_dict=ib_settings)
    prepared = []
    if not poc_levels.empty:
        prepared.append(prepare_poc_levels(poc_levels, ohlcv))
    if not ib_levels.empty:
        prepared.append(prepare_ib_levels(ib_levels, ohlcv))
    unified = pd.concat(prepared, ignore_index=True, sort=False) if prepared else pd.DataFrame()
    filtered = filter_unified_levels(unified, chart_settings)
    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Zobrazené levely", int(len(filtered)))
    c2.metric("Aktivní", int(filtered["Valid"].sum()) if not filtered.empty else 0)
    c3.metric("Testované", int((~filtered["Valid"]).sum()) if not filtered.empty else 0)
    c4.metric("POC", int((filtered["Source"] == "POC").sum()) if not filtered.empty else 0)
    c5.metric("IB", int((filtered["Source"] == "IB").sum()) if not filtered.empty else 0)
    c6.metric("Nejbližší distance", f"{filtered['SignedDist'].iloc[0]:+.2f}" if not filtered.empty else "n/a")
    st.plotly_chart(build_chart(ohlcv, filtered, ticker, chart_settings), use_container_width=True)
    st.subheader("Tabulka levelů")
    st.dataframe(build_table_view(filtered), use_container_width=True, hide_index=True)

if __name__ == "__main__":
    main()
