...
def main() -> None:
    ...
    if not poc_path.exists():
        print(f"❌ Soubor {POC_FILE} nenalezen. Nejdříve spusť POC_Calc.py")
        return

    poc_df = pd.read_csv(poc_path, parse_dates=["PeriodStart", "PeriodEnd"])
    required = {
        "Ticker",
        "PeriodType",
        "Period",
    ...
    missing = required - set(poc_df.columns)
    if missing:
        print(f"❌ POC CSV nemá očekávané sloupce: {sorted(missing)}")
        return

    tickers = poc_df["Ticker"].dropna().astype(str).unique().tolist()
    print(f"📊 Nalezeno {len(tickers)} tickerů v {POC_FILE}")
    ...
    sort_cols = [c for c in ["Ticker", "PeriodType", "PeriodStart"] if c in final.columns]
    if sort_cols:
